...
export default function Ticketing() {
  ...
  const handleInput = (idx, field, value) => {
    // Only allow numeric for count/price
    ...
    <div className="min-h-screen flex bg-black text-white font-[Ruda,sans-serif] pt-[85px]">
      ...
      <div className="flex-1 flex flex-col items-center p-8">
        ...
        >
          ...
          <div className="overflow-visible mb-6">
            ...
            >
              <thead>
                ...
                >
                  <th className="rounded-tl-lg px-2 py-2 text-xs font-semibold text-white border-r border-[#9C9C9C]" style={{height: "36px"}}>S. no.</th>
                  <th className="px-2 py-2 text-xs font-semibold text-white border-r border-[#9C9C9C]">Category Nam</th>
                  ...
                  <th className="px-2 py-2 text-xs font-semibold text-white">Price of ticket</th>
              ...
              <tbody>
                {rows.length === 0 ? null : rows.map((row, i) => (
                  <tr key={i} className="align-middle">
                    {/* Row boundary includes all cells except the last action cell */}
                    <td 
                      className={`px-2 py-1 text-center text-white font-medium align-middle ${i === 0 ? "rounded-bl-lg" : ""}`} 
                      style={{
                        background: "#000",
                    ...
                    {/* Last bordered cell in row boundary */}
                    <td 
                      className={`px-2 py-1 align-middle ${i === rows.length - 1 ? "rounded-br-lg" : ""}`}
                      style={{
                        background: "#000",
                        border: "1px solid #9D9D9D",
                        borderLeft: "none",
                    ...
                    {/* Action cell excluded from row boundary */}
              ...
              {platforms.map((platform) => (
                <label key={platform} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 accent-[#464646] rounded-sm"
                    style={{ accentColor: "#464646" }}
                  />
                  <span className={`font-semibold ${selectedPlatforms.includes(platform) ? "text-white" : "text-[#9C9C9C]"}`}>
                    {platform}
                  </span>
                </label>
              ))}
          ...
          {/* Event Layout / Sitting Plan and Confirm Button */}
          <div className="flex items-center gap-4 mt-8">
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                />
                <div
                  className="flex items-center bg-transparent border border-[#9C9C9C] rounded-lg px-4"
                  style={{ width: "669px", height: "55px", color: uploadName ? "#fff" : "#464646" }}
                  onClick={() => layoutInputRef.current?.click()}
                >
                  <span className={`flex-1 truncate text-base ${uploadName ? "text-white" : "text-[#464646]"}`}>
                    {uploadName || "Upload doc"}
                  </span>
                  <svg width="20" height="20" fill="none" className="ml-2 text-[#888]"><path d="M10 4v8m0 0l-3-3m3 3l3-3M4 16h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
              </div>
            </div>
            <button
              className="h-12 px-8 rounded-lg text-white font-semibold bg-gradient-to-r from-[#338EEF] to-[#0FE6FC] whitespace-nowrap"
            >
              Confirm Tickets
            </button>
          </div>
